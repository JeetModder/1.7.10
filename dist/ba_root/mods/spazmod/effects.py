...
class premiumsBallFactory(object):
    def __init__(self):
        self.bonesTex = ba.gettexture("powerupCurse")
        self.bonesModel = ba.getmodel("bonesHead")
        self.bearTex = ba.gettexture("bearColor")
        ...
        self.pballMaterial = ba.Material()
        self.pballMaterial.add_actions(actions=("modify_node_collision", "collide", False))


class premiumsBall(ba.Actor):
    def __init__(self, spaz, shape="bones"):
        ba.Actor.__init__(self)
        self.spazRef = weakref.ref(spaz)
        ...
        self.node = ba.newnode("prop",
                        attrs={"model": ba.getmodel("shield"),
                               ...
                               "velocity": (0, 0, 0),
                               "materials": [SharedObjects.get().object_material, factory.pballMaterial]}, delegate=self)
        ...
        1.2: (2,0,0)},
        loop = True)
        self.premiumsTimer = None
        self.premiumsRadius = 1.0
        self.angleDelta = math.pi / 12.0
        self.curAngle = random.random() * math.pi * 2.0
        self.curHeight = 0.0
        self.curHeightDir = 1
    ...
    def getTargetPosition(self, spazPos):
        p = spazPos
        pt = (p[0] + self.premiumsRadius * math.cos(self.curAngle), p[1] + self.curHeight, p[2] + self.premiumsRadius * math.sin(self.curAngle))
        self.curAngle += self.angleDelta
        self.curHeight += self.heightDelta * self.curHeightDir
        if (self.curHeight > self.heightMax) or (self.curHeight < self.heightMin): self.curHeightDir = -self.curHeightDir
        return pt
    ...
    def initTimer(self, p):
        self.node.position = self.getTargetPosition(p)
        self.premiumsTimer = ba.Timer(30, self.circleMove, repeat=True,  timetype=tt, timeformat=tf)
    ...
    def handlemessage(self, m):
        ba.Actor.handlemessage(self, m)
        if isinstance(m, ba.DieMessage):
            if self.premiumsTimer is not None: self.premiumsTimer = None
            self.node.delete()
        elif isinstance(m, ba.OutOfBoundsMessage):
            self.handlemessage(ba.DieMessage())
    ...
    def getFactory(cls):
        ...
        if activity is None: raise Exception("no current activity")
        try:
            return activity._sharedpremiumsBallFactory
        except Exception:
            f = activity._sharedpremiumsBallFactory = premiumsBallFactory()
            return f
...
class Effect(ba.Actor):
        ...
        try:
            if cl_str in custom_effects:
                ...
                if effect == 'ice':
                    ...
                    self.rainboweffect_timer = ba.timer(2, self.rainboweffect, repeat=True)
                elif effect == 'premiums':
                    self.premiums = premiumsBall(spaz, shape="bones")
    ...
    def emitSpark(self):
        spaz = self.spazRef()
        if spaz is None or not spaz.node.exists():
            self.handlemessage(ba.DieMessage())
            return
    ...
    def handlemessage(self, m):
        ...
        if isinstance(m, ba.OutOfBoundsMessage): self.handlemessage(ba.DieMessage())
            ...
            if hasattr(self, "rainboweffect"):self.rainboweffect_timer = None
            if hasattr(self, "premiums"):self.premiums = None
