...
async def refresh_stats():
    ...
    while not client.is_closed(): 
        part = get_live_players_msg()
        if "pb" in part:
            part = part.replace("", ("<:bs_v2account:999902124035026985>"))
            part = part.replace("", ("<:emoji_31:994568797212114984>"))
            part = part.replace("", ("<:bs_googleplay:991645800885125191>"))
            part = part.replace("", ("<:bs_andriod:991645838369640470>"))
        f_emb = discord.Embed(title="", description=part, color=discord.Colour.blue())
        
        await livestatsmsgs[0].edit(content=None, embed=f_emb)
...
def get_live_players_msg():
    global stats
    msg_1='***Players in server***\n\nOwner - <:bs_crown:991646978666680350>Jeet<:bs_crown:991646978666680350>\n\n'
    msg=''
    try:
    ...
    if not msg:
        msg="```No one``` \n"
    msg_2="\n\n***Current: *** "+stats['playlist']['current'] +"\n ***Next: ***"+stats['playlist']['next'] +"\n\n<:bs_viking:999902003264233542> IP - gvsb.cf\n<:bs_viking:999902003264233542> PORT - 43210"
