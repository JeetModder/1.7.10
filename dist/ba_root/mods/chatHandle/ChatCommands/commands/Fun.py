...
Commands = ['fly', 'invisible', 'headless', 'creepy', 'celebrate', 'spaz', 'speed', 'floater', '3dfly', 'hug', 'shower', 'box', 'mine', 'egg', 'ch']
CommandAliases = ['inv', 'hl', 'creep', 'celeb', 'flo', 'spray', 'meteor', 'drop']
...
def ExcelCommand(command, arguments, clientid, accountid):
	...
	if command=='speed':
	...
	elif command in ['shower', 'spray', 'drop', 'meteor']:
		shower(arguments)
		...
		ch(clientid, accountid, arguments)				
...
def ch(clientid,accountid,arguments):
    if arguments == [] or arguments == ['']:
        return
...
def shower(arguments):
	if arguments == [] or arguments == ['']:
		return
	bomb_list = random.choice(["normal", "ice", "sticky", "tnt", "land_mine"])
	activity = _ba.get_foreground_host_activity()
	if arguments[0] == 'all':
		for players in activity.players:
			pos = (players.actor.node.position[0], players.actor.node.position[1], players.actor.node.position[2])
			vel = (players.actor.node.velocity[0], players.actor.node.velocity[1], players.actor.node.velocity[2])
			bb = 0
			times = int(arguments[1])
			while bb < times:
				ba.timer(bb, ba.Call(shower_bomb, pos, vel, bomb_list))
				bb += 1
	else:
		num = int(arguments[0])
		player = activity.players[num]
		pos = (player.actor.node.position[0], player.actor.node.position[1], player.actor.node.position[2])
		vel = (player.actor.node.velocity[0], player.actor.node.velocity[1], player.actor.node.velocity[2])
		bb = 0
		times = int(arguments[1])
		while bb < times:
			ba.timer(bb, ba.Call(shower_bomb, pos, vel, bomb_list))
...
def shower_bomb(position: Sequence[float], velocity: Sequence[float], bomb_type: str) -> None:
    Bomb(position=position, velocity=velocity, bomb_type=bomb_type).autoretain()