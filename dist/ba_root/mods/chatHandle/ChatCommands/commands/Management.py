...
Commands = ['lm', 'gp', 'party', 'quit', 'kickvote','maxplayers','playlist','ban','kick', 'remove', 'end', 'quit', 'mute', 'unmute', 'slowmo', 'nv', 'dv', 'pause', 'cameramode', 'createrole', 'addrole', 'removerole', 'addcommand', 'addcmd', 'removecommand','getroles', 'removecmd', 'changetag','customtag','customeffect','add', 'spectators', 'lobbytime', 'mute10', 'disco']
	...
	try:
		...
		for ros in _ba.get_game_roster():
			if ros["client_id"]==cl_id:
				...
				_ba.chatmessage("Player muted for 10 mins")
				ba.timer(600, ba.Call(un_mute, arguments))
				_ba.chatmessage("Awake")
				ac_id=ros['account_id']
		if ac_id in serverdata.clients:
			serverdata.clients[ac_id]["isMuted"]=True
			ba.screenmessage("Player muted for 10 mins")
			ba.timer(600, ba.Call(un_mute, arguments))
...
def disco(arguments):
	...
	if arguments == [] or arguments == ['']:
		times=[0]
def discoRed():
	activity.globalsnode.tint = (1,.6,.6)
	ba.gameTimer(230,ba.Call(discoBlue))
def discoBlue():
	activity.globalsnode.tint = (.6,1,.6)
	ba.gameTimer(230,ba.Call(discoGreen))
def discoGreen():
	activity.globalsnode.tint = (.6,.6,1)
	if times[0]<10:
		times[0]+=1

	ba.gameTimer(230,ba.Call(discoRed))